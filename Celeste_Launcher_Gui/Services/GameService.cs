...
namespace Celeste_Launcher_Gui.Services
{
    static class GameService
    {
        ...
        public static async Task StartGame(bool isOffline = false)
        {
                ...
                if (LegacyBootstrapper.UserConfig.MpSettings.ConnectionType == ConnectionType.Wan)
                {
                    ...
                    if (LegacyBootstrapper.UserConfig.MpSettings.PortMappingType == PortMappingType.Upnp)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            GenericMessageDialog.Show(Properties.Resources.UPnPDisabledError, DialogIcon.Error, DialogOptions.ViewLog, LogHelper.GetLogFilePath());
                        }
                        finally
                        {
            ...
            {
                Logger.Error(exception, exception.Message);
                GenericMessageDialog.Show(Properties.Resources.StartGameError, DialogIcon.Error, DialogOptions.ViewLog, LogHelper.GetLogFilePath());
            }
        }
